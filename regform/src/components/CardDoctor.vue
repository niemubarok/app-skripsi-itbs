...
<template>
  <q-card class="my-card q-ma-sm q-pt-xl" style="
    ...
    " :style="$q.screen.gt.xs ? 'width:300px;' : ''" :class="disable(time.start) ? 'bg-grey-6 no-border' : ''">
    ...
    <q-card-section style="margin-top: -25px">
      <div class="text-subtitle1 text-center" style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap">
        <span class="text-accent text-light" style="text-overflow: ellipsis; overflow: hidden">
          {{ doctor }}
        </span>
      </div>
      <div class="column items-center">
        <!-- <div class="col items-center"> -->
        <q-chip style="text-align: center" :style="$q.screen.lt.sm ? 'margin-right:-10%' : ''"
          class="bg-grey-9 text-center text-light q-px-sm q-py-sm q-mt-sm" rounded :label="specialist"
    ...
    <q-card-actions>
      ...
      <q-space />
      <q-btn v-if="!$route.params.id" :to="'/registration/' + id" class="text-capitalize" style="width: 100px" push
        rounded text-color="primary" :color="disable(time.start) ? 'accent' : 'secondary'"
        :label="disable(time.start) ? 'Tutup' : 'Pilih'" @click="daftar" :disable="disable(time.start)" />
      <!-- {{ disable(time.start) }} -->
