<template>
  <transition appear enter-active-class="animated fadeInDown" leave-active-class="animated fadeOut">
    <div class="col-md-8 q-ml-sm">
      <q-card v-if="$route.params.id" class="transparent q-mt-sm q-pa-sm no-shadow flat" style="border-radius: 30px"
        :style="$q.screen.gt.sm ? 'border-left: 1px solid grey' : ''">
        <!-- chip -->
        <div style="margin-bottom: -15px">
          <q-chip color="secondary" text-color="white">
            <q-avatar color="primary" text-color="accent"><strong>2</strong></q-avatar>
            Isi Data Pasien
          </q-chip>
        </div>
        ...
        <div class="row no-wrap items-center">
          <q-card flat style="border-radius: 30px" class="bg-grey-3 q-pa-sm q-pb-md q-mt-md">
            <q-form @submit="onClickBtnCheckPatientData">
              <div>
                <!-- BASIC DATA PASIEN MUNCUL JIKA PASIEN ADALAH PASIEN BARU -->
                <q-card v-if="hideBasicForm" style="border-radius: 20px" class="q-pa-md bg-grey-3">
                  <div>
                    <q-badge no-caps class="no-shadow text-grey-9" color="grey-" rounded floating size="xs"
                      @click="editBasicData">edit <q-icon name="edit" /></q-badge>
                  </div>
                  <table>
                ...
                <div v-if="!hideBasicForm">
                  <!-- NIK -->
                  <q-input dense v-model="store.patient.detail.nik" mask="################" :rules="formRules.nik"
                    class="q-mt-sm" label="NIK / No. KTP" flat />

                  <!-- NAMA LENGKAP  -->
                  <q-input ref="inputNik" dense v-model="store.patient.detail.name" class="q-mt-sm" flat
                    label="Nama Lengkap Pasien" hint="Pastikan Sesuai dengan KTP" :rules="formRules.nama" />

                  <!-- TANGGAL LAHIR -->
                  <q-input dense class="q-mt-sm" v-model="store.patient.detail.birthDate" :rules="formRules.tglLahir"
                    :hint="$q.platform.is.desktop
                      ? 'Tanggal Lahir (contoh: 20/09/1992)'
                      : ''
                      " mask="##/##/####" :label="!$q.platform.is.desktop ? 'Tanggal Lahir' : ''" type="date">
                  </q-input>

                  <!-- WHATSAPP -->
                  <!-- <q-input dense v-model="store.patient.detail.phone" mask="####-####-####" unmasked-value class="q-mt-sm"
                    flat label="No. Whatsapp" :rules="formRules.HP"
                  ...
                  <div class="q-mt-lg">
                    ...
                    <div>
                      <q-radio v-model="store.patient.detail.jnsBayar" label="BPJS" val="BPJS">
                      </q-radio>
                      <q-radio v-model="store.patient.detail.jnsBayar" label="UMUM" val="UMUM" color="green">
                      </q-radio>
                      <q-radio v-model="store.patient.detail.jnsBayar" label="ASURANSI" val="ASURANSI" color="blue">
                      </q-radio>
                    </div>
                  </div>
                  <!-- JIKA PASIEN PILIH BPJS, MUNCULKAN KOLOM NOMOR KARTU BPJS -->
                  <div>
                    <q-input dense v-if="store.patient.detail.jnsBayar == 'BPJS'" v-model="store.patient.detail.noBPJS"
                      label="Nomor Kartu BPJS" mask="######################" :rules="formRules.jnsBayar"></q-input>
                  </div>
                </div>

                <!-- TOMBOL CEK APAKAH PASIEN SUDAH PERNAH BEROBAT ATAU BELUM -->
                <q-btn v-morph:btn:group:300.resize="morphBtnCheck" class="q-mt-md bg-primary" rounded
                  :class="btnCheckClass" label="Selanjutnya" type="submit" :disable="store.patient.detail.nik == '' ||
                    store.patient.detail.name == ''
                    ">
                  <q-spinner-cube v-if="isSearching" color="secondary" size="md" class="q-ml-sm" />
                </q-btn>

                <!-- MORPH KETIKA PASIEN ADALAH PASIEN BARU -->
                <q-card ref="cardFormPasienBaru" style="boder-radius: 30px; width: 95%" class="transparent" flat
                  v-morph:cardPasienBaru:group:300.tween="morphBtnCheck">
                  <div class="q-mt-md">
                    <q-chip class="bg-red-5 text-grey-2">Yuk! Lengkapi Data Pasien dulu (hanya sekali)</q-chip>
                  </div>
          ...
          .then((res) => {
            ...
            if (res.status == 200) {
              if (res.data.data !== null) {
                ...
                store.patient.detail.name = res.data.data.pasien.nm_pasien;
