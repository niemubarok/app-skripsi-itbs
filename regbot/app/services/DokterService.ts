...
export default class DokterService {
    ...
    public static async getDokterByName(name: string) {
        ...
        if (!dokterDiTabelJadwal) {
            return ""
            throw new Error("No dokter found");
        }
    ...
    public static async jadwalDokterPerTanggal(phoneNumber: number, namaDokter: string) {
        ...
        const [[dokter]] = await Database.rawQuery(
            ...
            "select distinct(dokter.nm_dokter), jadwal.kd_dokter, jadwal.kd_poli, jadwal.hari_kerja, jadwal.jam_mulai, jadwal.jam_selesai from jadwal inner join dokter on jadwal.kd_dokter = dokter.kd_dokter where jadwal.kd_dokter = ? and jadwal.hari_kerja = ?",
            [kd_dokter, Helpers.dateToDayForDB(tanggalPeriksa)]
        )
        return dokter
    }
}
