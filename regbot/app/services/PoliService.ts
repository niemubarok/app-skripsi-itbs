...
export default class PoliService {
  ...
  public static async jadwalPoli(phoneNumber: number, index: number = 0) {
    ...
    if (!namaPoli || !kodePoli) return null
    ...
    try {
      ...
      console.error("error di JadwalPoli", error);
      return null
    }
  }
  ...
  public static async jadwalPoliPerTanggal(phoneNumber: number) {
    ...
    try {
      ...
      _jadwalPoli.forEach((jadwal: any) => {
        if (!uniqueDoctors.has(jadwal.nm_dokter)) {
          uniqueDoctors.add(jadwal.nm_dokter);

          const _jadwal = _jadwalPoli
            .filter(
              (item: {
                [x: string]: any;
                hari_kerja: string;
      ...
      console.log("jadwalPoli", jadwalPoli.length);

      if (jadwalPoli.length === 0) {
        const poli: any = await PoliService.getPoliByLikeNama(namaPoli);
        Helpers.setCache("data poli", JSON.stringify(poli), phoneNumber);
        const jadwal = await this.jadwalPoli(phoneNumber);

        return `Mohon maaf tidak ada jadwal ${namaPoli} untuk hari ${hari}, ${Helpers.formatDateForDisplay(tanggalPeriksa)}\nBerikut jadwal ${jadwal?.namaPoli}:\n${jadwal?.jadwalPoli}`;

      } else {
        return `Berikut jadwal ${namaPoli} untuk ${hari}, ${Helpers.formatDateForDisplay(tanggalPeriksa)}\n${jadwalPoli}`;
      }

    } catch (error) {
      console.error("error di jadwalPoliPerTanggal", error);
      throw error;
    }
  }
  ...
  public static async getPoliByLikeNama(namaPoli: string) {
    try {
      ...
      console.error("error di getPoliByLikeNama", error);
  ...
  public static async getPoliByNama(namaPoli: string) {
    try {
      ...
      console.error("error di getPoliByNama", error);
    }
  }
  ...
  public static async getPoliByKode(kodePoli: string) {
    try {
      ...
      console.error("error di getPoliByKode", error);
    }
  }
